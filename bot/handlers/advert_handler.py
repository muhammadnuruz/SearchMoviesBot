...
from bot.buttons.reply_buttons import back_main_menu_buttons, admin_buttons
...
async def send_user_msg(msg: types.Message, state: FSMContext):
    ...
    try:
        await msg.copy_to(chat_id=int(data['user']), caption=msg.caption,
                          caption_entities=msg.caption_entities,
                          reply_markup=msg.reply_markup)
        await msg.answer(text="<b>✅ Xabar yetib bordi</b>", parse_mode="HTML", reply_markup=await admin_buttons())
    except ChatNotFound:
